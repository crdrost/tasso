...
interface IValidationOptions {
  /**
   * By default a missing property on an object or enum will generate a type error but not
   ...
   * or units.
   */
  jsonStrings?: Set<'number' | 'unit' | 'text' | 'object' | 'enum' | 'list'>;
}
...
export default function validate<tso extends TypeObject<Env>, Env extends TSchema>(
...
): ValidationOutput<IEval<tso, Env>> {
  const opts = options || {};
   ...
   * factored out here because it has to happen for both `object` types and `enum` types.
  ...
  function validateProps<props extends Record<string, TypeObject<Env>>>(
    ...
    if (!skipPropCheck) {
      ...
      const skipTypeKey = context.type === 'enum' ? context.typeKey : undefined;
  ...
  function validateOne<t extends TypeObject<Env>>(
  ...
  ): ValidationOutput<ValueOfType<t>> {
    // convenience method for emitting a single validation error.
    const singleErr = (msg: string): ValidationOutput<ValueOfType<t>> => {
    ...
    switch (typeObject.type) {
      ...
      case 'enum': {
        // to validate an enum type we need to ensure that the typeKey and valueKey exist on the
        // output, that the string mapped to by the type
        const {typeKey, options} = typeObject as IEnumerated<Env>;
        if (
          typeof value !== 'object' ||
